package com.bloomware.graze.config

import com.intellij.openapi.components.*
import com.intellij.util.xmlb.XmlSerializerUtil

/**
 * Persistent configuration service for Graze plugin that stores JIRA and Azure DevOps settings.
 * Uses IntelliJ's persistent state component to securely store configuration data.
 */
@State(
    name = "GrazeConfiguration",
    storages = [Storage("GrazeConfiguration.xml")]
)
@Service
class GrazeConfigurationService : PersistentStateComponent<GrazeConfigurationService.State> {
    ...
    companion object {
        fun getInstance(): GrazeConfigurationService {
            return service<GrazeConfigurationService>()
        }
    }
