...
import com.bloomware.graze.services.AzureDevOpsService
import com.bloomware.graze.services.GitService
import com.bloomware.graze.services.JiraService
...
/**
 * Main tool window for the Graze plugin.
 * Shows JIRA tickets, allows branch creation, and PR creation.
 */
class GrazeToolWindow(private val project: Project) {
                ...
                if (!isSelected && value is String) {
                    when {
                        ...
                        value.contains("Progress", true) || value.contains("Development", true) -> {
                            background = java.awt.Color(173, 216, 230) // Light blue
                        }
                        value.contains("Open", true) || value.contains("To Do", true) -> {
                            background = java.awt.Color(255, 255, 224) // Light yellow
                        }
                    }
                }
    ...
    private fun showSettings() {
        val dialog = GrazeSettingsDialog(project)
    ...
    private fun isConfigured(): Boolean {
        val config = GrazeConfigurationService.getInstance()
